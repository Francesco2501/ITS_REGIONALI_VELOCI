...
void UART_TxTempHumDataPacket(unsigned char a[7]);
void UART_TxBoolDataPacket(unsigned char a[4]);
...
unsigned char i;
unsigned char boolDataPacketTx[4];
unsigned char tempHumDataPacketTx[7];
unsigned char dataPacketRx[3];
...
int main()
{
    ...
    while(1)
    {
        if(flags &= 1<<0)
        {
            ...
            if(receivedData == 0xFF || partCounter > 3)
            {
                partCounter = 0;
                HandleRequest();
            }
...
void UART_TxTempHumDataPacket(unsigned char dataPacket[7])
{
    for(i=0;i<7;i++)
    {
...
void UART_TxBoolDataPacket(unsigned char dataPacket[4])
{
    for(i=0;i<4;i++)
    {
        UART_TxChar(dataPacket[i]);
...
void PrepareTempHumDataPacket(float temp, float hum)
{
    tempHumDataPacketTx[0] = 1;
    tempHumDataPacketTx[1] = 0x01;
